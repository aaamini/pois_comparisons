# DAVID INOUYE: Modified to also return alpha values
PGM.network.mod <-
function(X,R=max(X),nlams=10,lambda=NULL,parallel=TRUE,ncores=ncores){
	...
	wrapper <- function(i){
		...
		fit = PGM.path.neighborhood.mod(t(X[-i,]),X[i,],nlams=nlams,startb=0,lambda=lambda)
